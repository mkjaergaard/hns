...
TEST(BufferTest, Raw)
{
  ...
  hns::shared_buffer buffer_2 = boost::make_shared<hns::raw_buffer>(&data_1[0], 1024);
...
TEST(BufferTest, Stacked)
{
  ...
  hns::outbound_data<hns::boost_serializer, uint32_t> o_data_2(120);
  hns::outbound_pair o_data(o_data_2, o_data_1);

  // Create buffer and pack data
  hns::shared_buffer buffer = boost::make_shared<hns::raw_buffer>(&data_1[0], 1024);
  o_data.pack(buffer);

  // Unpack data
  hns::inbound_data<hns::boost_serializer, uint32_t> in_val_2(buffer);
